...
namespace agio
{
    struct Settings
    {
		...
		inline static float MinSpeciesAge = 20;

		// Number of individuals to consider when computing progress metrics
		inline static float ProgressMetricsIndividuals = 5;
		...
		inline static float SpeciesStagnancyChances = 1e10;//10;

		// Minimum number of individuals allowed on a species
		inline static float MinIndividualsPerSpecies = 50;

		// Number of times to try when creating a new morphology
		inline static float MorphologyTries = 10;

		// Number of replications to do when simulating
		inline static float SimulationReplications = 10;
